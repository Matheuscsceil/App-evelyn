...
export default function DetailSerie({ navigation }) {
  ...
  useEffect(() => {
    navigation.setParams({ title: serie.title });
  ...
  async function addListWatched(type, text) {
    ...
    try {
      ...
      if (series_evelyn == null) {
        ...
        if (filter.length == 0) {
          await store.save("series-evelyn", [...series_evelyn, { ...serie, type: 1 }]);
        } else {
          const result = series_evelyn.map(item => (item.id == serie.id) ? { ...item, avaliation: serie.avaliation, type: 1 } : item);
  ...
  async function addListNotWatched(type, text) {
    ...
    try {
      ...
      if (series_evelyn == null) {
        ...
        if (filter.length == 0) {
          await store.save("series-evelyn", [...series_evelyn, { ...serie, type: 2 }]);
        } else {
          const result = series_evelyn.map(item => (item.id == serie.id) ? { ...item, avaliation: serie.avaliation, type: 2 } : item);
          await store.save("series-evelyn", result);
        }
      }
    } catch (error) {
  ...
  async function changeAvaliation() {
    ...
    if (series_evelyn != null) {
      ...
      if (filter.length > 0) {
        const result = series_evelyn.map(item => (item.id == serie.id) ? { ...item, avaliation: serie.avaliation } : item);
        await store.save("series-evelyn", result);
      }
    }
          ...
          <View style={{ borderRadius: 25, backgroundColor: "#fff", padding: 20 }}>
            ...
            <View>
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                <Text style={{ color: colors.primary, fontFamily: "Quicksand-Bold" }}>Configurações da Série</Text>
      ...
      <View>
        <Modal isVisible={modalAvaliationVisible}>
          <View style={{ borderRadius: 25, backgroundColor: "#fff", padding: 20 }}>
            ...
            <Text style={{ fontFamily: "Quicksand-Bold", color: colors.primary }}>NOTA: {Number(serie.avaliation)}</Text>
            <Slider
              ...
              onValueChange={value => {
                setSerie({ ...serie, avaliation: Number(value * 10).toFixed(1) })
              }}
            />
