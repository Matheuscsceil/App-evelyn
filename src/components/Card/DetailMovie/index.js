...
export default function DetailMovie({ navigation }) {
  ...
  useEffect(() => {
    navigation.setParams({ title: extenseName(movie.title) });
  ...
  async function addListWatched(type, text) {
    ...
    try {
      ...
      if (movies_evelyn == null) {
        ...
        if (filter.length == 0) {
          await store.save('movies-evelyn', [
            ...movies_evelyn,
            { ...movie, type: 1 },
          ]);
        } else {
          const result = movies_evelyn.map((item) =>
            item.id == movie.id
              ? { ...item, avaliation: movie.avaliation, type: 1 }
              : item
          );
    ...
    try {
      ...
      if (movies_evelyn == null) {
        ...
        if (filter.length == 0) {
          ...
          const result = movies_evelyn.map((item) =>
            item.id == movie.id
              ? { ...item, avaliation: movie.avaliation, type: 2 }
              : item
          );
          await store.save('movies-evelyn', result);
        }
  ...
  async function changeAvaliation() {
    ...
    if (movies_evelyn != null) {
      ...
      if (filter.length > 0) {
        const result = movies_evelyn.map((item) =>
          item.id == movie.id ? { ...item, avaliation: movie.avaliation } : item
        );
        await store.save('movies-evelyn', result);
      }
    }
